...
#endif  
   if ( events & HAL_STEP_MOTOR_EVT ) //步进电机步进事件
  {
    hal_step_motor_drive();
    return (events ^ HAL_STEP_MOTOR_EVT);
  } 
  
    if ( events & HAL_BODY_DETECT_EVT )  //人体检测事件
  {
    hal_body_detect_state_machine();  // 人体检测推理机
    return (events ^ HAL_BODY_DETECT_EVT);
  }  
  
  // 光敏电阻
  if ( events & HAL_LIGHT_DETECT_EVT ) {
    hal_light_detect_state_machine();
    return (events ^ HAL_LIGHT_DETECT_EVT);
  }
  
    if ( events & HAL_LIGTH_DELAY_SCAN_EVT ) {
    hal_finish_light_event_process();
    return (events ^ HAL_LIGTH_DELAY_SCAN_EVT);
  }
  // 开关切换
  if ( events & HAL_SWITCH_DETECT_EVT ) {
    hal_switch_detect_state_machine();
    return (events ^ HAL_SWITCH_DETECT_EVT);
  }
 
    if ( events & HAL_SCAN_DELAY_FINISH_EVT )  // 扫描延时结束事件
  {
    hal_scan_delay_event_process();  // 延时结束事件处理
    return (events ^ HAL_SCAN_DELAY_FINISH_EVT);
  }  
  
   if ( events & HAL_STEP_MOTOR_ROUND_TRIP_FINISH_EVT )
  {
    hal_auto_scan_event_process(); // 扫描结束后处理自动扫描事件
    return (events ^ HAL_STEP_MOTOR_ROUND_TRIP_FINISH_EVT);
  } 
